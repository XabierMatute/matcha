            ...
            try:
                with connection.cursor() as cursor:
                    cursor.execute(
                        ...
                        (email, username, password)  # Aquí debes usar un hash para la contraseña
                    )
                    connection.commit()
    ...
    if request.method == 'POST':
        ...
        if connection:
            with connection.cursor() as cursor:
                ...
                user = cursor.fetchone()
                if user and user[2] == password:  # Verificar la contraseña, usa hashing aquí en un entorno real
